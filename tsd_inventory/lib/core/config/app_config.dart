...
class AppConfig {
  ...
  /// Используется для запросов к 1С (например, `inventoryPath('updateFact')`).
  /// Автообновление здесь больше не проходит: оно читается из публичной папки
  /// Яндекс Диска (см. `core/update`).
