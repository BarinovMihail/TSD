...
releases/
  tsd-inventory-<version>-<code>.apk       # APK (по умолчанию)
  # либо tsd-inventory-<version>-<code>.zip  (APK, запакованный в zip)
```
Приложение умеет скачивать **и** `.apk`, **и** `.zip` (формат определяется по
расширению в `apkPath` манифеста; zip распаковывается автоматически).
...
```json
{
  "versionName": "0.2.6",
  "versionCode": 8,
  "apkPath": "releases/tsd-inventory-0.2.6-8.apk",
...
`apkPath` — путь к файлу APK относительно публичной папки. Допустимы `.apk`
(готовый файл) или `.zip` (APK внутри архива — распаковывается автоматически).
SHA-256 считается по **итоговому APK**. Парсинг безопасный: некорректный
...
После скачивания приложение получает готовый APK (для `.apk` — напрямую, для
`.zip` — после распаковки), считает его SHA-256 и сравнивает с `sha256` из
манифеста без учёта регистра. При несовпадении файлы **удаляются**, установка не
...
# по умолчанию публикует готовый .apk; для zip добавьте -Format zip
```
Скрипт сам собирает release-APK, считает SHA-256, публикует APK (`-Format apk`
по умолчанию) или zip (`-Format zip`), генерирует `manifest.json` (`apkPath`,
`sha256`, `required`) и заливает всё в папку Диска: **сначала APK/zip, потом
manifest.json** (чтобы клиенты не увидели манифест, ссылающийся на ещё не
залитый файл). Тот же ключ release-подписи обязателен.

Шаги вручную (если без скрипта): собрать release-APK, посчитать SHA-256
(`certutil -hashfile ... SHA256`), при желании упаковать в zip, обновить
`manifest.json` (шаблон — `dist/manifest.json`, в `apkPath` — `.apk` или `.zip`),
залить APK/zip **раньше** `manifest.json`.
